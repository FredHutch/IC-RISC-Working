...
beacon= read_excel("./input_data/beagess_IC-RISC.xlsx")
...
beacon_imp$statins= ifelse(beacon_imp$cc==0,
                           cut(statins, c(0,.65,1), labels= c(1,0)),  #OR=0.57; prev(contr)=.35
                           cut(statins, c(0,.765,1), labels= c(1,0)))
beacon_imp$family.history= ifelse(beacon_imp$cc==0,
                                  cut(family.history, c(0, 0.972, 1), labels= c(0,1)),
                                  cut(family.history, c(0, 0.927, 1), labels= c(0,1))) #OR=2.76; prev(case)=7.3%
